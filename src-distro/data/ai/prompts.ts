...
export const prompts = {
    ...
    recordIntoChatSimplified: (context: PromptContext) => {
        return 'Structured health record (RecordId: ' + context.record?.id + ', date: ' + context.record?.eventDate + ') in JSON:  \r\n\r\n```json\
        \r\n' + JSON.stringify(context.record?.json) + '```'
    },
